...
class CallForPaperController {
  ...
  static getCallForPaper : RequestHandler = async (req, res) => {
    try {
      ...
      if (!callForPaper)
        return res.status(404).json({ msg: `No Event with id: ${callForPaperID}` });
  ...
  static deleteCallForPaper : RequestHandler = async (req, res) => {
    try {
      const { id: callForPaperID } = req.params;
      const callForPaper = await CallForPaper.findOneAndDelete({ _id: callForPaperID });
      if (!callForPaper)
        return res.status(404).json({ msg: `No Event with id: ${callForPaperID}` });
      return res.status(200).json({ callForPaper });
    } catch (error) {
  ...
  static updateCallForPaper : RequestHandler = async (req, res) => {
    try {
      const { id: callForPaperID } = req.params;
      const callForPaper = await CallForPaper.findOneAndUpdate({ _id: callForPaperID }, req.body);
      if (!callForPaper)
        return res.status(404).json({ msg: `No Event with id: ${callForPaperID}` });
      return res.status(200).json({ callForPaper });
    } catch (error) {
